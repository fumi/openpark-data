...
RDF::Writer.open(OUTPUT_FILE, :prefixes => PREFIXES) do |writer|
  CSV.foreach(INPUT_FILE) do |row|
    ...
    graph = RDF::Graph.new
    graph.from_ttl("park:#{row[1]} a schema:Park, schema:CivicStructure, schema:Place, schema:Thing, ic:地点, ic:施設 ;
                      rdfs:label \"#{row[1]}\"@ja  ;
                      schema:address [ a schema:PostalAddress ;
                        schema:postalCode \"#{row[3]}\" ;
                        schema:streetAddress \"#{row[4].gsub('神奈川県横浜市金沢区', '')}\"@ja ;
                        schema:addressRegion \"横浜市金沢区\"@ja ;
                        schema:addressLocality \"神奈川県\"@ja ;
                      ] ;
